...
- **Блюр пропадает при открытии другой вкладки в режиме unstuck (несколько вкладок рядом).**
  При переключении/открытии другой вкладки, когда вкладки не сгруппированы, размытие приватной
  заметки слетает. Предполагаемый корень: CSS-гейт завязан на `.is-active` внутри
  `.workspace-tabs:has(.is-active [data-link-tags*='#private'])` (`styles.scss`) — блюрится только
  активный лист, соседние/неактивные приватные заметки не размываются. Чинить.
