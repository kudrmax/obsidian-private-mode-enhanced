...
Содержимое `#private-always` всегда размыто в hover-preview, embed и индексированных выдержках;
их заголовки следуют глобальному поведению `#private`, а у напрямую открытой вкладки сохраняется
локальное поведение `#private-always`.
...
- **`secondary-render-protection.ts`** — чистая политика direct/secondary, резолвинг путей
  индексированных результатов и изолированное состояние hover-preview.
- **`secondary-render-controller.ts`** — единый lifecycle-адаптер для Markdown-renderer,
  hover-preview и индексированных результатов; ставит семантический класс защиты.
